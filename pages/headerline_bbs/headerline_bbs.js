...
Page({
  ...
  onLoad: function (option) {
    ...
    // //二维码
    // var url = util.GetXCXInfoQRCode('id=' + option.id, 'pages/headerline_bbs/headerline_bbs', app.globalData.loginInfo.siteId, function (url) {
    //   wx.getImageInfo({
    //     src: url,
  ...
  // 获取详情
  GetBssInfoData: function () {

    var that = this
    let methodName = "PHSocket_XCX_GetXCXBBSTopicInfo";
    //console.log('id' + that.data.detailId)
    var params = util.requestParam(methodName, { siteID: app.globalData.loginInfo.siteId, topicID: that.data.detailId });
    ...
    util.request({
      ...
      success: function (res) {
        // var newUrl = res.ServerInfo.detaileData.Images.length > 0 ? res.ServerInfo.detaileData.Images[0].replace('http://imgref.ccoo.cn/ajax/imgref.ashx?dis=_500x300(w)&url=', '') : ''
        if (res.MessageList.code == 1000) {
          ...
          that.setData({
            ...
            sharePic: res.ServerInfo.detaileData.Images.length > 0 ? res.ServerInfo.detaileData.Images[0] : "http://img.pccoo.cn/headline-xcx/images/pop-ban.jpg",
  ...
  //内容过多时，请求内容的下半部分
  GetBssInfoDataPart: function () {

    var that = this
    let methodName = "PHSocket_XCX_GetXCXBBSTopicMemoPart";
    //console.log('id' + that.data.detailId)
    var params = util.requestParam(methodName, { siteID: app.globalData.loginInfo.siteId, topicID: that.data.detailId, num: that.data.num});
